...
export function JourneyAtlas() {
  ...
  return <section id="atlas" className="atlas-section">
    ...
    <div className="atlas-layout">
      ...
      <article className={`scene-card scene-${current.image}`}><img className="atlas-scene-image" src={sceneSources[current.image]} alt={`${current.title} 场景插画`}/><div className="scene-overlay"><p>{current.year} · {current.place}</p><h3>{current.title}</h3><span>{current.age}</span></div></article>
