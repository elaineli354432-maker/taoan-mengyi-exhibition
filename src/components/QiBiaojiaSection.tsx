...
export function QiBiaojiaSection() {
  ...
  return <section id="qi-biaojia" className="qi-act-page">
    <header className="qi-act-head">
      <div className="qi-act-head-image" style={{ backgroundImage: `linear-gradient(90deg, rgba(12, 24, 26, .96) 0%, rgba(12, 24, 26, .65) 42%, rgba(12, 24, 26, .12) 76%), url('${assetUrl('/images/qi-dream-shikui-whisper.png')}')` }} aria-hidden="true"/>
    ...
    <div className="qi-act-scenes" aria-label="祁世培入梦五组图像素材">
      {qiDreamSection.scenes.map((item, index) => <article key={`${item.year}-${item.title}`} className="qi-act-scene">
        <img src={assetUrl(item.image)} alt={`${item.title} 场景插画`}/>
    ...
    <aside className="lu-qi-bridge">
      <img src={assetUrl('/images/qi-return-to-writing.png')} alt="张岱归山著史的山路场景"/>
    ...
    <div id="writing-history" className="writing-transition" style={{ backgroundImage: `linear-gradient(90deg, rgba(11, 22, 24, .88), rgba(11, 22, 24, .3)), url('${assetUrl('/images/qi-return-to-writing.png')}')` }}>
